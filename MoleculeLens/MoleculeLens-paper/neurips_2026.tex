...
\begin{abstract}
Despite rapid progress in drug--text alignment, the field lacks a principled framework for understanding \emph{what molecular features} drive cross-modal retrieval decisions, \emph{where} in a text encoder mechanism-relevant information emerges, and \emph{how much} of apparent alignment is genuine structure--mechanism correspondence versus lexical shortcutting through drug-name leakage. We propose \textit{MoleculeLens}, a framework for mechanistic interpretability of drug--text alignment, built around a contrastive bridge that connects frozen ECFP4 fingerprints and a frozen biomedical sentence encoder via two learned linear projection heads in a shared 256-dimensional space. The linear design is not a simplification compromise: it is the enabling condition for exact, closed-form attribution of any retrieval decision to individual ECFP4 molecular substructures via a single matrix product, with no approximation, sampling, or gradient estimation.

We ground the framework in a reproducible ChEMBL benchmark of 2{,}699 approved-drug mechanism pairs under a Bemis--Murcko scaffold-disjoint split (2{,}019 train / 245 val / 435 held-out test). MoleculeLens achieves Recall@1 of 0.179 and MRR of 0.270, outperforming MolPrompt (0.129 / 0.256), KV-PLM (0.055 / 0.121), and zero-shot Graphormer (0.002 / 0.015); bootstrap 95\% CIs are non-overlapping between MoleculeLens $[0.156, 0.228]$ and MolPrompt $[0.101, 0.164]$, and five-seed variance yields mean Recall@1 $0.213 \pm 0.045$, confirming that competitive performance does not require sacrificing interpretability.

MoleculeLens yields four mechanistic findings inaccessible to black-box models. \textbf{(i) Attribution validity:} ECFP4 attribution case studies across eight mechanism families confirm that high-attribution bits decode to pharmacophore-relevant atoms, establishing that the shared space organises around chemical meaning rather than scaffold artefacts. \textbf{(ii) Leakage anatomy:} A Jaccard-overlap analysis reveals that 39\% of attribution fingerprints shift substantially when drug names are removed, providing the first substructure-level account of why name leakage inflates retrieval scores. \textbf{(iii) Representation depth:} A \textit{contrastive logit lens}---applying the trained text projection at each of the 12 RoBERTa layers---shows that drug-retrievable information crystallises in a single discontinuous jump at the final layer; the entire leakage gap of 0.124 is also concentrated at layer~12, ruling out layer-specific mitigation. \textbf{(iv) Error structure:} Wrong-but-close retrieval errors show mean attribution correlation 0.835, confirming that model confusions are pharmacologically structured rather than arbitrary.

Together, these findings establish MoleculeLens as a mechanistic interpretability framework for cross-modal drug retrieval: the linear bridge makes every alignment decision auditable at the level of individual molecular substructures and transformer layers.
\end{abstract}

\section{Introduction}

Drug--text alignment asks whether a molecular representation and a natural-language description of mechanism or therapeutic function can be mapped into a common retrieval space. A robust solution would support mechanism-based search, structured pharmacology retrieval, and retrieval-augmented drug discovery workflows. In practice, however, strong retrieval scores can emerge for the wrong reasons: drug names can leak identity into text, global train--test splits allow memorization of scaffold-correlated representations, and benchmark choices can conflate semantic alignment with surface-form matching. Beyond these evaluation concerns lies a deeper question that existing models leave unanswered: \emph{which molecular substructures actually drive the alignment}, and to what extent does the model reflect genuine structure--mechanism correspondence rather than incidental lexical cues?

We address this by proposing \textit{MoleculeLens}, a mechanistic interpretability framework for cross-modal drug--text alignment. Rather than treating the alignment model as a black box to be evaluated only by retrieval rank, MoleculeLens is designed from the ground up to make every retrieval decision auditable. The enabling design choice is that both modality-specific projection heads are purely linear, which allows any alignment decision to be decomposed exactly into contributions from individual ECFP4 molecular substructures in closed form. No post-hoc explanation method, sparse autoencoder, or gradient approximation is needed: the attribution is a direct consequence of the linear structure. The model is evaluated on a scaffold-disjoint ChEMBL benchmark to ensure that interpretability findings reflect genuine structure--mechanism correspondence rather than memorisation of training scaffolds.

This paper makes four concrete contributions:
\begin{enumerate}
  \item \textbf{Mechanistic interpretability framework.} We introduce exact ECFP4 substructure attribution for cross-modal contrastive retrieval and demonstrate that the shared space organises around pharmacophore-relevant molecular features across eight mechanism families.
  \item \textbf{Leakage anatomy.} We provide the first substructure-level account of drug-name leakage, showing via Jaccard-overlap analysis that 39\% of retrieval decisions rely on attribution patterns that do not survive drug-name removal---leakage is not merely a scoring artefact but a concrete change in which molecular features the model treats as important.
  \item \textbf{Contrastive logit lens.} We introduce a layer-wise analysis that locates where drug-retrievable information emerges in a frozen biomedical text encoder, finding a single discontinuous jump at the final transformer layer with the entire leakage gap concentrated at the same depth.
  \item \textbf{Reproducible scaffold benchmark.} We construct and release a Bemis--Murcko scaffold-disjoint retrieval benchmark from 2{,}699 approved-drug ChEMBL mechanism pairs, evaluate MoleculeLens against MolPrompt, KV-PLM, and a zero-shot Graphormer baseline, and confirm statistical reliability via bootstrap CIs and five-seed variance analysis.
\end{enumerate}
