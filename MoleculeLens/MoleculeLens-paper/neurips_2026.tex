...
\begin{abstract}
Drug--text retrieval systems can connect molecular structures to mechanism descriptions, but most current models are difficult to interpret mechanistically. They often rely on large black-box encoders, and benchmark scores can be inflated by memorized scaffolds, drug names, or target metadata. As a result, it is unclear whether a retrieved match is supported by meaningful molecular substructures or by easier lexical shortcuts. We present \textit{MoleculeLens}, an interpretable drug--text retrieval framework that makes each alignment decision traceable to salient molecular features. MoleculeLens aligns fixed molecular fingerprints and frozen biomedical text embeddings using learned linear projection heads. This deliberately constrained architecture is the core methodological contribution: the molecule-side linear map efficiently ranks which ECFP4 substructure bits support a retrieval, avoiding a rerun for every masked bit. On held-out scaffold pairs, this ranking agrees with two exact but slower checks: differentiating the normalized retrieval score (mean Pearson 0.905) and recomputing the score after removing each present bit (0.947). We evaluate MoleculeLens on a reproducible ChEMBL benchmark of 2{,}699 approved-drug mechanism pairs using a Bemis--Murcko scaffold-disjoint split. MoleculeLens achieves the strongest Recall@1 and MRR among the evaluated cross-modal baselines on this split, while also exposing when performance depends on drug-name or target/action metadata. Beyond retrieval accuracy, MoleculeLens provides pharmacophore-level explanations for correct matches, faithful diagnostics for failed retrievals, and a same-model substructure-level analysis of lexical leakage. These results show that lightweight linear bridges can make drug--text alignment more interpretable and chemically grounded without giving up competitive top-rank retrieval performance.
\end{abstract}
...
\begin{itemize}
\item \textbf{A deliberately interpretable retrieval model.} MoleculeLens keeps the molecular fingerprints and biomedical text encoder fixed and trains only two linear bridges. This design makes each drug--text score directly explainable as ranked ECFP4 fingerprint environments, so the model can show which chemical substructures support a proposed mechanism match.
...
Drug--text and molecule--language models include Text2Mol for retrieval~\citep{edwards2021text2mol}, MoleculeSTM for structure--text retrieval and editing~\citep{liu2023moleculestm}, MolT5 and BioT5 for molecular language modeling~\citep{edwards2022molt5,pei2023biot5}, MolCA for graph--language alignment~\citep{liu2023molca}, and KV-PLM for biomedical multimodal pretraining~\citep{zeng2022kvplm}. Graphormer~\citep{ying2021graphormer}, Uni-Mol~\citep{zhou2023unimol}, ChemBERTa~\citep{chithrananda2020chemberta}, and graph pretraining~\citep{hu2020pretraining} are important molecular representation lines, though not direct text-query retrieval systems without additional alignment. MoleculeLens is complementary to higher-capacity models and molecular contrastive systems such as MolCLR, SMICLR, and CLOOME~\citep{wang2022molclr,pinheiro2022smiclr,sanchezfernandez2023cloome}: it emphasizes scaffold-disjoint drug--mechanism retrieval and substructure-level mechanistic explanations. The evaluation design follows the molecular-learning practice of scaffold splits~\citep{bemis1996frameworks} and combines it with text leakage controls.
...
\begin{figure}[!htbp]
  \centering
  \makebox[\linewidth][c]{\includegraphics[width=1.02\linewidth]{fig_architecture.pdf}}
