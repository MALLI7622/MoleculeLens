...
\begin{abstract}
We introduce \textit{MoleculeLens}, a framework for mechanistic interpretability of drug--text alignment. Cross-modal retrieval models that map molecular structure to mechanism-of-action text are typically evaluated as black boxes; MoleculeLens is designed to make every retrieval decision auditable at the level of individual molecular substructures. The framework connects frozen ECFP4 fingerprints and a frozen biomedical sentence encoder via learned linear projection heads in a shared embedding space---an architectural choice that enables exact, closed-form attribution of any alignment decision to specific molecular fragments without approximation or gradient estimation. We ground the framework in a reproducible ChEMBL benchmark of approved-drug mechanism pairs under a Bemis--Murcko scaffold-disjoint split, and demonstrate that the interpretable design remains competitive against stronger baselines. Mechanistic analysis reveals which substructures drive correct retrieval across diverse mechanism families, quantifies the substructure-level footprint of drug-name leakage, and identifies the precise transformer layer at which drug-retrievable information crystallises in the frozen text encoder. Together, these analyses establish MoleculeLens as a transparent and reproducible foundation for understanding what molecular features drive drug--text alignment, and why current benchmarks can overstate progress.
\end{abstract}
