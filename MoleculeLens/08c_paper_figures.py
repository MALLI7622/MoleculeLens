...
def trim_whitespace(img, pad=4):
    """Crop a rendered molecule to its non-background content."""
    if img is None:
        return None
    rgb = img.convert("RGB")
    ...
    lower = min(lower + pad, rgb.height)
...
img_r  = trim_whitespace(render_mol(smiles_lk, atom_cols_r,  size=(250, 180)), pad=2)
img_nd = trim_whitespace(render_mol(smiles_lk, atom_cols_nd, size=(250, 180)), pad=2)

fig2 = plt.figure(figsize=(10.6, 3.15))
gs2 = gridspec.GridSpec(
    1, 5, figure=fig2, wspace=0.30,
    left=0.055, right=0.995, bottom=0.20, top=0.84,
    width_ratios=[1.22, 0.03, 1.45, 0.03, 1.45]
)
...
ax_hist.set_xlabel("Jaccard overlap of top-10\nattribution bits (rich vs nodrug)",
                   fontsize=8)
ax_hist.set_ylabel("# test pairs", fontsize=8)
...
for col_idx, (bits, vals, label, correct_str, bar_col) in enumerate([
    ...
    inset = ax.inset_axes([0.57, 0.49, 0.35, 0.43])
...
save_and_mirror(fig2, "fig_leakage_substructure", dpi=300, pad_inches=0.005)
plt.close(fig2)
...
gs3 = gridspec.GridSpec(
    1, 2, figure=fig3, wspace=0.12,
    left=0.060, right=0.995, bottom=0.20, top=0.84,
    width_ratios=[1.00, 2.15]
)
...
ax_corr.set_xlabel("Attribution correlation\n(correct drug vs retrieved drug)",
                   fontsize=8)
ax_corr.set_ylabel("# rank-2/3 error pairs", fontsize=8)
ax_corr.set_title("(A) Are rank-2/3 errors\nchemically reasonable?",
                  fontsize=9, fontweight="bold")
ax_corr.tick_params(axis="both", labelsize=7)
...
img_c   = trim_whitespace(render_mol(s_c, ac_c, size=(260, 180)), pad=2)
img_r   = trim_whitespace(render_mol(s_r, ac_r, size=(260, 180)), pad=2)

inner3 = gridspec.GridSpecFromSubplotSpec(1, 3, subplot_spec=gs3[1],
                                          width_ratios=[1, 0.16, 1], wspace=0.0)
...
for ax, img, label in [
    ...
    if img:
        ax.imshow(img)
    ax.axis("off")
    ax.set_title(label, fontsize=8.5, fontweight="bold", pad=2)
...
save_and_mirror(fig3, "fig_wrong_close", dpi=300, pad_inches=0.005)
plt.close(fig3)
