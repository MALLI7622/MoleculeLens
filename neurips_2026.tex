...
\begin{abstract}
Aligning drug representations with mechanism-of-action text is useful for retrieval, hypothesis generation, and biomedical search, but current benchmarks can overstate progress when lexical cues or train--test overlap make retrieval artificially easy. This paper presents \textit{MoleculeLens}, a scaffold-split extension of Thin Bridges for drug--text alignment on ChEMBL mechanism pairs. MoleculeLens uses frozen text representations, ECFP4 molecular fingerprints, and lightweight linear projections trained with a contrastive objective. The evaluation is built from 2,699 approved-drug mechanism pairs from ChEMBL and a reproducible Bemis--Murcko scaffold split with 2,019 train, 245 validation, and 435 held-out test examples. On the held-out scaffold test set, MoleculeLens reaches Recall@1 of 0.179 and MRR of 0.270, outperforming MolPrompt (0.129 Recall@1, 0.256 MRR), KV-PLM (0.055 Recall@1, 0.121 MRR), and a zero-shot Graphormer baseline (0.002 Recall@1, 0.015 MRR). Removing drug names from the text reduces MoleculeLens Recall@1 from 0.179 to 0.106, a smaller relative drop than the earlier full-gallery memorization-heavy Thin Bridges setting. Because both projection heads are purely linear, retrieval decisions can be attributed exactly to individual ECFP4 molecular substructures in closed form. Case studies across eight mechanism families confirm that high-attribution bits correspond to pharmacophore-relevant atoms; a Jaccard-overlap analysis reveals that 39\% of test pairs rely on attribution patterns that do not survive drug-name removal; and wrong-but-close errors show mean attribution correlation 0.835, confirming that retrieval confusions are chemically grounded. These results suggest that thin contrastive bridges remain competitive under scaffold-disjoint evaluation and are uniquely amenable to exact mechanistic interpretability.
\end{abstract}
