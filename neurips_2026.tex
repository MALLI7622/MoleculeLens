...
The task is bidirectional retrieval: given a text query, retrieve the matching drug structure from a gallery, and given a structure query, retrieve the matching text. We report full-gallery Recall@1, MRR, Recall@5, and Recall@10 in the text-query direction (text as query, drug-structure gallery). This direction aligns naturally with the attribution and logit-lens analyses in Sections~7--8, where a mechanism text is the starting point for each retrieval decision. We also retain T-choose-one retrieval as a secondary diagnostic for earlier-experiment compatibility.
...
\textbf{MoleculeLens (ours)} & Scaffold test ($N{=}435$) & \textbf{0.191} & \textbf{0.284} & 0.377 & 0.476 \\
...
\textbf{MoleculeLens (ours)} & Scaffold test ($N{=}435$) & \textbf{0.191} & \textbf{0.094} & \textbf{0.097} & \textbf{50.6} \\
...
\begin{enumerate}
  ...
  \item \textbf{Drug-name leakage is visible at the substructure level.} The mean Jaccard overlap between top-attribution bits under \texttt{text\_rich} and \texttt{text\_nodrug} is only 0.262, and 39\% of pairs have overlap below 0.2. The 50.6\% Recall@1 drop from leakage maps onto a concrete shift in which molecular features the model treats as important.
...
Figure~\ref{fig:logit-lens} and Table~\ref{tab:logit-lens} summarise the results across 435 test pairs. The layer-12 Recall@1 for \texttt{text\_rich} ($0.170$) is slightly lower than in Table~\ref{tab:main-results} ($0.191$) because the logit lens re-encodes all texts through fresh RoBERTa forward passes rather than using cached $B^{(t)}_{\text{test}}$ embeddings; minor floating-point differences accumulate across 12 transformer layers. The relative pattern---all retrieval ability concentrated at layer~12, with the full leakage gap appearing at the same depth---is robust to this difference.
